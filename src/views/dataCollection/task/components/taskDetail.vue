<template>
  <div>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="ruleStandard" label="设计要求与规范规定" align="center" />
      <el-table-column prop="sampleAmount" label="样本总数" width="100" align="center" />
      <el-table-column prop="minSample" label="最小抽样批量" width="200" align="center" />
      ...
      <el-table-column label="操作" align="center" width="200">
        <template slot-scope="scope">
          <el-button type="warning" size="small" @click="editCapactity(scope)">编辑</el-button>
          <el-button type="danger" size="small" @click="resetCapactity(scope)">重置</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="选择试验报告编号" :visible.sync="dialogEdit" :show-close="false" @close="resetDialog()">
      <div class="filter-container">
        ...
        <el-input v-model="otherInfo" placeholder="请输入内容" style="width: 10em;" />
      </div>

      <el-table ref="multipleTable" :data="reportData" @selection-change="handleSelectionChange">
        <el-table-column type="selection" fixed />
        <el-table-column label="台账编号" prop="dataId" align="center" />
  ...
  methods: {
    ...
    async getRepotData() {
      ...
      try {
        ...
        const res = await getFinishedList(this.curReportType, this.curPage, this.limit, subprojectId)
        if (res.code == 200) {
    ...
    async submitData() {
      ...
      try {
        const res = await submitData(resultData)
        if (res.code == 200) {
